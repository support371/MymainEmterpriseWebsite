...
export default function AdminOperations() {
  ...
  const refresh = () => {
    adminApi.operations().then((d) => setOpsLog(d.operations)).catch(() => {});
    adminApi.emailOutbox().then((d) => setEmailOutbox(d.outbox)).catch(() => {});
  };

  useEffect(() => { refresh(); }, []);

  const handleRunEmailWorker = async () => {
    try {
      const result = await adminApi.processEmails();
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={handleRunEmailWorker} className="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white text-sm font-medium rounded-lg transition-colors">Run Email Worker</button>
        </div>
          ...
          <tbody className="divide-y">
            {opsLog.length === 0 ? (
              ...
              <tr key={op.id} className="hover:bg-gray-50">
                <td className="px-4 py-3"><span className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-xs">{op.type}</span></td>
