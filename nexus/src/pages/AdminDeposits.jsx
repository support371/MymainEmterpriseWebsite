...
export default function AdminDeposits() {
  ...
  const refresh = () => adminApi.deposits().then((d) => setDeposits(d.deposits)).catch(() => {});
  useEffect(() => { refresh(); }, []);

  const handleSimulate = async (id) => {
    try {
      await adminApi.simulateInbound(id);
  ...
  const handleSettle = async (id) => {
    try {
      await adminApi.settleDeposit(id);
              ...
              <tr key={d.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    {d.status === 'requested' && <button onClick={() => handleSimulate(d.id)} className="text-xs px-2 py-1 bg-blue-100 text-blue-700 rounded hover:bg-blue-200 transition-colors">Simulate Inbound</button>}
                    {d.status === 'received' && <button onClick={() => handleSettle(d.id)} className="text-xs px-2 py-1 bg-green-100 text-green-700 rounded hover:bg-green-200 transition-colors">Settle</button>}
                    {d.status === 'complete' && <span className="text-xs text-gray-400">Done</span>}
                  </div>
                </td>
              </tr>
            ))}
