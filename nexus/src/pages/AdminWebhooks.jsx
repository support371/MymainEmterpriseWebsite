...
export default function AdminWebhooks() {
  ...
  useEffect(() => {
    adminApi.webhooks().then((d) => setEvents(d.events)).catch(() => {});
  }, []);
            ...
            {events.length === 0 ? (
              ...
              <React.Fragment key={e.id}>
                <tr className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3"><button onClick={() => toggle(e.id)} className="text-xs text-orange-600 hover:underline font-medium">{expandedId === e.id ? 'Hide' : 'View'}</button></td>
                </tr>
                {expandedId === e.id && (
                  <tr><td colSpan={5} className="px-4 py-3 bg-gray-50">
                    <pre className="text-xs font-mono bg-gray-900 text-green-300 p-4 rounded-lg overflow-x-auto">{formatJson(e.payload)}</pre>
                  </td></tr>
                )}
              </React.Fragment>
            ))}
...
function formatJson(str) {
  try { return JSON.stringify(JSON.parse(str), null, 2); } catch { return str || '(empty)'; }
}
