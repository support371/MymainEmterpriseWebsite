...
export default function AdminClients({ onNavigate }) {
  ...
  useEffect(() => {
    adminApi.clients().then((d) => setClients(d.clients)).catch(() => {});
  }, []);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-4">
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name or email..." className="w-full max-w-md px-3 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" />
      </div>
            ...
            {filtered.length === 0 ? (
              ...
              <tr key={c.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <button onClick={() => onNavigate('admin-client-detail', { clientId: c.id })} className="text-orange-600 hover:underline text-xs font-medium">View Detail</button>
                </td>
              </tr>
            ))}
          </tbody>
