...
export default function AdminCards() {
  ...
  const refresh = () => {
    adminApi.cardRequests().then((d) => setRequests(d.requests)).catch(() => {});
    adminApi.cards().then((d) => setCards(d.cards)).catch(() => {});
  };
  ...
  const handleApprove = async (requestId) => {
    try {
      await adminApi.approveCard(requestId);
      toast.success('Card approved and issued');
      refresh();
    } catch (err) { toast.error(err.message); }
  };

  const handleSimulateTx = async (cardId) => {
    try {
      const amt = parseFloat(txAmount) || 50;
      await adminApi.simulateTransaction(cardId, { amount: amt, merchant });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
              ...
              {requests.length === 0 ? (
                ...
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    {r.status === 'requested' && <button onClick={() => handleApprove(r.id)} className="text-xs px-2 py-1 bg-green-100 text-green-700 rounded hover:bg-green-200 transition-colors">Approve & Issue</button>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="bg-gray-50 border rounded-lg p-4 mb-4 flex flex-wrap gap-3 items-end">
        <div><label className="block text-xs font-medium text-gray-600 mb-1">Merchant</label><input type="text" value={merchant} onChange={(e) => setMerchant(e.target.value)} className="px-2 py-1 text-sm border rounded" /></div>
        <div><label className="block text-xs font-medium text-gray-600 mb-1">Amount (USD)</label><input type="number" value={txAmount} onChange={(e) => setTxAmount(e.target.value)} className="px-2 py-1 text-sm border rounded w-24" /></div>
      </div>
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
          ...
          <tbody className="divide-y">
            {cards.length === 0 ? (
              ...
              <tr key={c.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  {c.status === 'active' && <button onClick={() => handleSimulateTx(c.id)} className="text-xs px-2 py-1 bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors">Simulate Tx</button>}
                </td>
              </tr>
            ))}
          </tbody>
