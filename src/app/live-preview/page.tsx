...
const backendEndpoints = [
  ...
  { method: 'POST', path: '/api/admin/logout', description: 'Admin session termination endpoint.' },
  { method: 'GET',  path: '/api/admin/inbox',  description: 'Secured admin message retrieval endpoint.' },
];
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="relative overflow-hidden border-b border-slate-800/70 pt-24">
        ...
        <div className="container relative mx-auto px-4 pb-16">
          ...
          <p className="mt-5 max-w-3xl text-slate-300">
            ...
            stakeholders can review both customer-facing positioning, backend readiness, and scaling coverage
            in one live surface.
          </p>
      ...
      <section className="py-14">
        <div className="container mx-auto grid gap-8 px-4 lg:grid-cols-2 lg:items-start">

          {/* LEFT — Complete Route Coverage (all routes, no scroll cap) */}
          <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
            <h2 className="text-2xl font-semibold">Complete Route Coverage</h2>
            <p className="mt-2 text-sm text-slate-400">
              All customer, platform, admin, and legal routes tracked for enterprise release validation.
            </p>
          ...
          <div className="space-y-8">
            ...
            <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
              ...
              <p className="mt-2 text-sm text-slate-400">
                All delivery services included for enterprise-grade scale review.
              </p>
              <div className="mt-5 space-y-3">
                {serviceModules.map((svc) => (
                  <div key={svc.path} className="rounded-lg border border-slate-800 bg-slate-950/60 px-3 py-2">
                    ...
                    <p className="mt-1 text-xs text-slate-400">
                      <span className="font-semibold text-slate-200">{svc.label}</span>
                      <br />
                      {svc.description}
                    </p>
                  </div>
            ...
            <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
              ...
              <p className="mt-2 text-sm text-slate-400">
                Administrative service surfaces included in delivery readiness.
              </p>
                  ...
                  <div key={redirect.source} className="flex items-start gap-3 rounded-lg border border-slate-800 bg-slate-950/60 px-3 py-2 text-sm">
                    ...
                    <div>
                      <p>
                        ...
                        <span className="text-slate-500">({redirect.permanent ? '301' : '307'} · {redirect.reason})</span>
                      </p>
                    </div>
                  </div>
                ))}
